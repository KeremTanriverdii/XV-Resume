...
export const PaddleSubscribeModal: React.FC<PaddleSubscribeModalProps> = ({
  ...
  const handleSubscribe = () => {
    ...
    if (!selectedPriceId) {
      alert(
        'Paddle Price ID is missing in environment variables. Please check your .env.local!',
      );
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-white/10 bg-slate-900 p-6 sm:p-8 shadow-2xl">
        ...
        <div className="text-center mb-6">
          ...
          <p className="mt-2 text-sm text-slate-400">{t('upgradeSubtitle')}</p>
        </div>
        ...
        >
          {loading
            ? t('openingCheckout')
            : billingCycle === 'monthly'
              ? t('subscribeNowMonthly')
              : t('subscribeNowYearly')}
        </Button>
