  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <div className="text-3xl font-semibold animate-pulse">{t('loading')}</div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="p-5 border rounded-2xl bg-card flex gap-4 items-start shadow-xs animate-pulse">
