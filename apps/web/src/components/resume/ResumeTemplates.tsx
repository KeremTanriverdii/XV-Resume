...
export const ResumeTemplates: React.FC<ResumeTemplatesProps> = ({
  ...
  const defaultLabels =
    SECTION_LABELS_BY_LANG[langCode] || SECTION_LABELS_BY_LANG.en;
  ...
  const formattedLocation = (loc?: string | null) => {
    ...
    if (langCode === 'en') {
      clean = clean
        .replace(/İstanbul/gi, 'Istanbul')
        .replace(/Türkiye/gi, 'Turkey');
    } else if (langCode === 'de') {
      clean = clean
        .replace(/İstanbul/gi, 'Istanbul')
        .replace(/Turkey|Türkiye/gi, 'Türkei');
    } else if (langCode === 'tr') {
      clean = clean
        .replace(/Istanbul/gi, 'İstanbul')
        .replace(/Turkey/gi, 'Türkiye');
    } else if (langCode === 'fr') {
      clean = clean
        .replace(/İstanbul/gi, 'Istanbul')
        .replace(/Turkey|Türkiye/gi, 'Turquie');
    } else if (langCode === 'es' || langCode === 'it') {
      clean = clean
        .replace(/İstanbul/gi, 'Istanbul')
        .replace(/Turkey|Türkiye/gi, 'Turquía');
    }
    return clean;
  };
  ...
  const renderSectionHeader = (
    label: string,
    styleVariant: 'modern' | 'executive' | 'sidebar' | 'minimal',
  ) => {
  ...
  const cleanExperienceHtml = (htmlContent?: string | null) => {
    ...
    return htmlContent
      ...
      .replace(
        /^<p>\s*(Teknik Yetkinlikler|Yetenekler|Skills|Technical Skills|İş Deneyimi|Work Experience|Eğitim|Education)\s*:?\s*<\/p>/gi,
        '',
      )
      .replace(
        /^<(h[1-4])>\s*(Teknik Yetkinlikler|Yetenekler|Skills|Technical Skills|İş Deneyimi|Work Experience|Eğitim|Education)\s*:?\s*<\/\1>/gi,
        '',
      )
      .replace(
        /^<strong>\s*(Teknik Yetkinlikler|Yetenekler|Skills|Technical Skills)\s*:?\s*<\/strong>\s*(<br\s*\/?>)?/gi,
        '',
      )
  ...
  const renderSocialLinks = (
    socialLinks?: string[],
    textClass: string = `${theme.primaryClass} hover:underline`,
  ) => {
    if (!socialLinks || socialLinks.length === 0) return null;
    const activeLinks = socialLinks.filter((l) => l && l.trim().length > 0);
  ...
  if (templateId === 'modern') {
    return (
      <div
        ...
        className="rounded-2xl border border-border/80 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 shadow-xl p-8 sm:p-12 font-sans min-h-[850px] flex flex-col gap-6 transition-all duration-300"
      >
        {/* Document Header */}
        <div
        ...
        >
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
            ...
            <div className="flex-1">
              <h2 className="text-2xl font-extrabold tracking-tight text-zinc-900 dark:text-white leading-none">
                {fullName}
              </h2>
              <p
                className={`text-sm font-semibold ${theme.primaryClass} mt-2 uppercase tracking-wide`}
              >
                {title}
              </p>
          ...
          <div className="flex flex-col text-xs text-zinc-500 dark:text-zinc-400 gap-1 sm:text-right font-medium shrink-0">
            ...
            {profile?.location && (
              <span>{formattedLocation(profile.location)}</span>
            )}
            {profile?.militaryStatus && profile.militaryStatus !== 'None' && (
              <span>
                {labels.military}:{' '}
                {labels.militaryStatus[profile.militaryStatus] ||
        ...
        {/* Sections */}
        {renderHtmlSection(
          translation.experienceHtml,
          labels.experience,
          'modern',
        )}
        {renderHtmlSection(
          translation.educationHtml,
          labels.education,
          'modern',
        )}
        {renderHtmlSection(
  ...
  if (templateId === 'executive') {
    return (
      <div
        id="cv-document-container"
        className="rounded-2xl border border-zinc-300 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 shadow-xl p-8 sm:p-12 font-serif min-h-[850px] flex flex-col gap-4"
      >
        {/* Executive Header */}
        <div className="text-center border-b-2 border-zinc-900 dark:border-zinc-100 pb-5">
          ...
          <div className="flex justify-center mt-2">
            {renderSocialLinks(
              profile?.socialLinks,
              'text-zinc-700 dark:text-zinc-300 hover:underline font-serif',
            )}
          </div>
        </div>
  ...
  if (templateId === 'sidebar') {
    return (
      <div
        id="cv-document-container"
        className="rounded-2xl border border-border/80 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 shadow-xl overflow-hidden font-sans min-h-[850px] grid grid-cols-1 md:grid-cols-12"
      >
        {/* Left Column */}
        <div
        ...
        >
          <div className="flex flex-col items-center text-center">
            {profile?.showPhoto && profile?.photoUrl ? (
              <img
                src={profile.photoUrl}
                alt={fullName}
                className="h-24 w-24 rounded-full object-cover border-2 border-white shadow-md mb-3"
              />
            ) : (
              <div
                className={`h-20 w-20 rounded-full ${theme.badgeClass} flex items-center justify-center text-2xl font-bold mb-3 shadow-xs`}
              >
                {fullName.charAt(0)}
              </div>
            )}
            ...
            <p
              className={`text-xs font-semibold ${theme.primaryClass} mt-1 uppercase tracking-wide`}
            >
              {title}
            </p>
          </div>
          ...
          <div className="space-y-2 text-xs font-medium text-zinc-650 dark:text-zinc-300 border-t border-border/60 pt-4">
            ...
            {renderSocialLinks(
              profile?.socialLinks,
              `${theme.textClass} hover:underline`,
            )}
          </div>
            ...
            <div className="space-y-2 border-t border-border/60 pt-4 cv-section-compact avoid-break [break-inside:avoid] [page-break-inside:avoid]">
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: cleanExperienceHtml(translation.skillsHtml),
                }}
              />
            </div>
          )}
          ...
          {translation.languagesHtml && (
            <div className="space-y-2 border-t border-border/60 pt-4 cv-section-compact avoid-break [break-inside:avoid] [page-break-inside:avoid]">
              ...
              <div
                className="prose prose-xs max-w-none text-zinc-700 dark:text-zinc-300 prose-ul:my-1 prose-ul:list-disc prose-ul:pl-4 prose-li:my-0.5 prose-p:my-0.5"
                dangerouslySetInnerHTML={{
                  __html: cleanExperienceHtml(translation.languagesHtml),
                }}
              />
            </div>
          )}
  ...
  return (
    <div
      id="cv-document-container"
      className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 shadow-xl p-8 sm:p-12 font-sans min-h-[850px] flex flex-col gap-5"
    >
      {/* Minimal Header */}
      <div className="flex flex-col sm:flex-row sm:items-baseline justify-between border-b border-zinc-200 dark:border-zinc-800 pb-4 gap-2">
        <div>
          ...
          {renderSocialLinks(
            profile?.socialLinks,
            'text-zinc-600 dark:text-zinc-400 hover:underline font-mono',
          )}
        </div>
